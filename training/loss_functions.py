...
def sound_loss(loss_data_dict, weights=None, device=None):
    ...
    # compute the loss
    bz_loss = torch.sum(torch.norm(torch.abs(bz_h) - torch.abs(des_bz_h), p=2, dim=1))
    dz_loss = torch.sum(torch.norm(torch.abs(dz_h) - torch.abs(des_dz_h), p=2, dim=1))

    loss = bz_loss + dz_loss

    loss_dict = {"bz_loss": bz_loss, "dz_loss": dz_loss, "loss": loss}
