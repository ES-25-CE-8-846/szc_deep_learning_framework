...
def plot_filters(ax, filters, name, sample_rate=48000):
    ...
    for fi in range(min(3, filters.shape[1], 1)):  # Plot up to 3 speakers
...
if __name__ == "__main__":
    ...
    for i, data_dict in tqdm(enumerate(torch_dataloader)):
        ...
        # ax.plot(bz_rirs[0,0,0,:])
        # ax[1].plot(dz_rirs[0,0,0,:])

        # ax.set_title("bz_rirs")

        # ax[1].set_title("dz_rirs")
        ...
        for n, filter_name in enumerate(baseline_filters):
            if filter_name == "dirac":
                ...
                ax.plot(filters_to_test[filter_name][0,:], alpha=0.01, c="b")
        ...
        if i > 100:
            plt.savefig("avg_pm.pdf", format="pdf",dpi=1200)
            plt.show()
            break
